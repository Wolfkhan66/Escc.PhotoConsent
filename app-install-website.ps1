...
robocopy $sourceFolder "$destinationFolder/$projectName" /MIR /IF *.dll *.css csc.* csi.* *.js *.cshtml *.asax *.gif *.scss *.png *.swf *.eot *.svg *.ttf *.woff *.woff2 /XD aspnet_client obj Properties App_Start
...
CreateVirtualDirectory $websiteName "PhotoConsent" "$destinationFolder\$projectName" true "$projectName-$websiteName"
DisableAnonymousAuthentication $websiteName "PhotoConsent"
EnableWindowsAuthentication $websiteName "PhotoConsent"
